...
\begin{frame}
	...
	\begin{block}{General approach applied on Forward Euler}
		...
		\begin{center}
			...
			\end{align*}
		\end{center}
		If we use the Forward Euler method, we have that s is replaces by $\frac{z-1}{T_s}$, so we can find the discrete-time equivalent as follows:
		\vspace{-2em}
		\begin{center}
			\begin{align*}
...
\begin{frame}
	...
	\begin{block}{Origin of prewarping}
		On the next slide you can see that when we compare the $\mathcal{Z}$-transform with the bilinear transformation, points b and d in the s-plane are mapped onto different points in the z-plane. $(-1 + 0j)$ can only be reached by the bilinear rule when $\omega$ equals infinity.
	\end{block}
\end{frame}
