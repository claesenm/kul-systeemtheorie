...
\begin{frame}
	\frametitle{Examples: Sensitivity to errors in the model}
	\begin{figure}
		\centering
		\includegraphics[width=0.7\linewidth]{control-goals/formula-1}
		\caption{It is not possible to drive a formula 1 car using your knowledge of regular cars. However, driving your friends car will work.}
		\label{fig:formula-1}
	\end{figure}
\end{frame}
...
\begin{frame}
	...
	\begin{block}{}
			In practice, the transfer function $P(s)$ is often unknown. It is important to know how model errors affect the performance of the controller. Sensitivity and robustness are key concepts to evaluate these effects.
			\begin{itemize}
				\item Sensitivity quantifies the effect of small model errors on the output.
				\item Robustness refers to bigger changes of the model. A controller is robust if it works properly over a given set of parameters.
			\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}
	...
	\begin{block}{}
		Sensitivity is a measure of the effect of a small changes of process parameters on the model output.
	...
	\frametitle{Example of a type 0 system}
		\begin{example}
			...
			\begin{flalign*}
				...
				&= \lim\limits_{s \rightarrow 0} \frac{A}{s + \color{red}{s \frac{K \prod_{k=1}^{m} (-z_k)}{\prod_{k=1}^{n-i} (-p_k)}}} = \frac{1}{\color{red}{K_v}} = \infty
			\end{flalign*}
