...
\begin{frame}
	\begin{exampleblock}{Example 1 (1/4)}
		...
		\end{figure}
		Based on the figure, we can conclude the following:
		\vspace{-1em} 
		\begin{equation}
			E(s) = R(s) - Y(s)
			\vspace{-1em}
...
\begin{frame}
	\begin{exampleblock}{Example 1 (3/4)}
		...
		\end{align*}
		The last expression is obtained by applying Relation II on the previous expression.
	\end{exampleblock}
\end{frame}
...
\begin{frame}
	\begin{exampleblock}{Example 1 (4/4)}
		\justify		
		We now that the resulting transfer function is also the transfer function of the following discrete-time system:
		\begin{figure}
			\centering
			\includegraphics[width=0.8\linewidth]{block_analysis_3}
		\end{figure}
		Consequently, we have found a discrete-time equivalent of a system which contained continuous-time and sampled signals.
	\end{exampleblock}
\end{frame}
...
\begin{frame}
	\begin{exampleblock}{Example 2 (2/3)}
		...
		Y^{*}(s) = G_p^{*}(s)D^{*}(s) \big ( R^{*}(s) - Y^{*}(s) \big)
		\end{equation}
		This is the exact same expression as we had for example 1. We will find the same transfer function by simplifying Eq.(17).
	\end{exampleblock}
\end{frame}
